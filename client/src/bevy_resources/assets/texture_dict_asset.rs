...
impl AssetLoader for TextureDictAssetLoader {
    ...
    async fn load(
        ...
        let parsed = &YamlLoader::load_from_str(&String::from_utf8_lossy(&bytes))
            .map_err(|e| InvalidConfig(e.to_string()))?[0];
...
fn parse_array_texture_params(
    params: &Yaml,
) -> Result<TextureArrayData, TextureDictAssetLoaderError> {
    use TextureDictAssetParseError::*;

    let path: String = params["path"]
        .as_str()
        .ok_or(InvalidConfig("missing texture path".to_string()))?
        .to_string();
    let texture_indexes: &Hash = params["textures"].as_hash().ok_or(InvalidConfig(
        "array texture must have defined texture indexes".to_string(),
    ))?;
    let mut textures = HashMap::new();

    for (k, v) in texture_indexes {
        let texture_name: &str = k
            .as_str()
            .ok_or(InvalidConfig("texture name should be a string".to_string()))?;
        let texture_index: i64 = v.as_i64().ok_or(InvalidConfig(
            "texture index should be an integer".to_string(),
        ))?;

        if texture_index < 0 {
            return Err(TextureDictAssetLoaderError::Parse(InvalidConfig(
                "texture index should be a valid u32".to_string(),
            )));
        }
...
fn parse_palette_texture_params(params: &Yaml) -> Result<PaletteData, TextureDictAssetLoaderError> {
    ...
    let path: String = params["path"]
        ...
        .ok_or(InvalidConfig("missing texture path".to_string()))?
        .to_string();
    let len: i64 = params["len"].as_i64().ok_or(InvalidConfig(
        "palette texture must have defined palette length".to_string(),
    ))?;

    if len < 0 {
        return Err(TextureDictAssetLoaderError::Parse(InvalidConfig(
            "len should be a valid u32".to_string(),
        )));
    }

    let len = len as usize;

    Ok(PaletteData { path, len })
}
