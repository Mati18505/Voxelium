...
impl Default for ChunkMeshDiagnosticsConfig {
    fn default() -> Self {
        Self { max_history_length: DEFAULT_MAX_HISTORY_LENGTH }
    }
}

#[derive(Default)]
pub struct ChunkMeshDiagnosticsPlugin (
    pub ChunkMeshDiagnosticsConfig
);
...
impl Plugin for ChunkMeshDiagnosticsPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(self.0.clone())
            .register_diagnostic(
                Diagnostic::new(Self::CHUNK_MESH_COUNT).with_max_history_length(self.0.max_history_length),
            )
            .register_diagnostic(
                Diagnostic::new(Self::VERTEX_COUNT).with_max_history_length(self.0.max_history_length),
            )
...
impl ChunkMeshDiagnosticsPlugin {
    ...
    pub fn diagnostic_system(
        ...
        let chunk_mesh_count: usize = chunk_meshes.0
            .iter()
            .count();

        let vertex_count: usize = chunk_meshes.0
            .iter()
