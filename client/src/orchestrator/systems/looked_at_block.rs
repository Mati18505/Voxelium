...
pub fn update_looked_at_block(
    ...
    if dirty {
        process_raycast_and_send_event(looked_at_block_change_ev, chunk_manager_resources, game_resources, looked_at_block_data);
    }
}

fn process_raycast_and_send_event(mut looked_at_block_change_ev: EventWriter<'_, LookedAtBlockChangedEvent>, chunk_manager_resources: Res<'_, ChunkManagerResources>, game_resources: Res<'_, GameResources>, looked_at_block_data: Mut<'_, LookedAtBlockData>) {
    let world = &chunk_manager_resources.chunk_manager.get_world().world;
    let raycast_result = raycast_from_controller(looked_at_block_data.last_player_pos, looked_at_block_data.last_looking_dir, world, &game_resources.server_block_type_storage);
    let block_pos = raycast_result.hitpoint.pos;
    let block_id = raycast_result.hitpoint.block_id;

    if let Some(default_block_type) = game_resources.server_block_type_storage.get_by_id(name_to_block_id("air")) {
        if raycast_result.collide {
            ...
            looked_at_block_change_ev.write(LookedAtBlockChangedEvent { block_pos, block_type: block_type.clone() });
        } else {
            looked_at_block_change_ev.write(LookedAtBlockChangedEvent { block_pos, block_type: default_block_type.clone() });
        }
    } else {
